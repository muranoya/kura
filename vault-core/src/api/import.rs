...
impl VaultManager {
    ...
    pub fn api_import_1pux_preview(&self, file_bytes: Vec<u8>) -> Result<String, String> {
        ...
        let preview =
            crate::import::preview::generate_preview(&parsed_items, &file_bytes, unlocked)
                .map_err(|e| format!("Failed to generate preview: {}", e))?;

        serde_json::to_string(&preview).map_err(|e| format!("Serialization error: {}", e))
    }
    ...
    pub fn api_import_1pux_execute(
        ...
        let actions: Vec<crate::import::ImportItemAction> = serde_json::from_str(&actions_json)
        ...
        if let Some(SessionState::Unlocked(ref mut unlocked)) = session.as_mut() {
            let result = crate::import::execute_import(&parsed_items, &actions, unlocked)
