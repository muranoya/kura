...
マッチングは以下の3層を並行評価し、結果を合併して候補提示する。

### パスA: Vault エントリの直接マッチ（`associated_packages`）

Vault エントリに `associated_packages` フィールドが設定されている場合、そのパッケージ名とリクエスト元が一致すれば直接候補に追加する。ユーザーが明示的に「このエントリはこのアプリ用」と紐付けたものであり、最も高い信頼度を持つ。

パスB と並行して評価し、両方の結果を合併する。パスA でマッチしたエントリはパスB からも重複排除される。

> **未実装（将来対応予定）**: `associated_packages` フィールドの実装は別スコープ。現時点ではパスA は常に空集合を返す。

### パスB: パッケージ名→ドメイン解決→ドメインベースマッチ
...
`LoginCandidateMatcher` は従来通り、ホストの正規化（`www.` 除去）とサフィックス一致でマッチングを行う。解决された各ドメインに対してマッチングを行い、結果を合併する。

- 未登録パッケージは候補を一切表示しない（誤マッチより「候補なし」を優先する安全側デフォルト）
- ヒューリスティックな推測（パッケージ名からドメインを機械的に導出する等）は行わない
...
「クレデンシャルの最小露出」原則を維持する。候補リスト取得の時点ではパスワードを含まない情報（URL・ユーザー名等）のみを扱い、Section 1-3の3層解決による絞り込みを行った上で、実際にマッチしたエントリについてのみ個別に復号する。

マッチングフローは以下の並行処理で構成される：

1. **パスA（直接マッチ）**: Vault エントリの`associated_packages`にリクエスト元パッケージ名が含まれるエントリを直接候補に追加（将来設計、Section 3-2-3参照）
2. **パスB（ドメイン推論マッチ）**: `DomainVerificationManager` → `package_domains.json`（フォールバック）の順でパッケージ名→ドメインを解決し、各ドメインで`LoginCandidateMatcher`によるURLマッチングを行う
3. 両パスの結果を合併し、重複排除して候補リストを構築

Android Autofillフレームワークの`Dataset`はフィールド値を`onFillRequest`のレスポンス構築時点で確定させる必要があり、ブラウザ拡張のように「候補表示後、選択された1件のみ復号する」という二段階を素朴には実現できない（`Dataset`ごとに個別の認証を設定し、選択時に値を確定させることも可能だが、都度認証を要求するとUXが悪化する）。そのため、**vaultアンロック済みの場合は候補提示の時点で全マッチ候補のパスワードをまとめて復号し`Dataset`に埋め込む方式を採用した**（3層解決で該当件数は通常少数のため許容）。選択時に個別復号する方式は、都度認証によるUX悪化を避けるため採用しなかった。
...
現時点でのvault-core側の変更は不要。ログイン候補一覧を取得するための既存APIをそのまま利用する。

将来、Section 3-2-3の`associated_packages`を実装する際は、vault-coreの`VaultEntry`にフィールド追加が必要となる。
...
## 3-2. パッケージ名からドメイン/アプリへの解決

Section 1-3の3層解決に対応する実装設計。
...
- 結果はプロセス生存中に不変のためキャッシュする

### 3-2-2. `package_domains.json`（フォールバック）

`DomainVerificationManager`でドメインが解決できなかった場合にのみ、手動キュレーションDBにフォールバックする。App Links未宣言だがウェブサイトとID/Passwordを共通するアプリ（例: 一部の金融アプリ、SaaS等）向けの補助手段。

**データ形式:** パッケージ名をキーとし、値を属性オブジェクト（ドメイン等）とする辞書形式。値をドメイン文字列そのものではなくオブジェクトにするのは、将来的な拡張（例: コメント、検証状況フラグ、複数ドメインを許容する場合の配列化等）をスキーマ非破壊で行えるようにするため。1ドメインに対して複数パッケージが対応するケース（Android版/iOS版で別パッケージ名、等）を考慮し、キーはパッケージ名側とする。

**配置場所:** `extension/patterns/`（拡張機能が自前キュレーションするサイトパターンDB）と同様の位置づけとして、Androidアプリ内リソース`assets/`に配置する。

### 3-2-3. `associated_packages`による直接マッチ（将来設計）

Vault エントリに`associated_packages: Option<Vec<String>>`フィールドを追加し、パッケージ名を直接紐付ける。ユーザーが明示的に「このエントリはこのアプリ用」と設定したものであり、最も高い信頼度を持つ。

- パスB（ドメイン解決）と並行して評価し、パスAの結果は常に候補に含める
- パスA でマッチしたエントリはパスB からも重複排除される
- vault-coreの`VaultEntry`にフィールド追加が必要（スキーマバージョンのインクリメントは不要 — フィールド追加のみのため前方互換性が保たれる）

> **未実装（将来対応予定）**: 本セクションの内容は将来の完全版実装方針であり、現時点では未実装。優先度は高くない。
