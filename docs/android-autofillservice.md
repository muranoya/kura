...
`AutofillService`はシステムのAutofillフレームワークからバインドされるコンポーネントであり、`BIND_AUTOFILL_SERVICE`権限の指定がシステム以外からのバインドを拒否するために必須となる。`minSdk 34`のため、AutofillFramework自体の可用性チェック（バージョン分岐）は不要。
...
**署名証明書fingerprintによるなりすまし対策:** パッケージ名自体は1-3節の通り実行中アプリが偽装することはできないが、Google Playと異なりサイドロードでは同一パッケージ名・異なる署名鍵のアプリの共存を妨げる仕組みが無い。そのため、ユーザーの端末に本物のアプリが入っていない状態で、攻撃者が同じパッケージ名を名乗る悪意あるapkをサイドロードさせる「パッケージ名squatting」攻撃が原理的に成立しうる。

これに対応するため、値オブジェクトに署名証明書のSHA-256 fingerprint（署名鍵ローテーションを考慮し複数登録可）を追加できるようにし、インストール済みアプリの実際の署名証明書と照合したうえでドメインを返す。**fingerprintが未登録・不一致のいずれの場合も、未登録パッケージと同じ安全側デフォルト（候補を一切出さない）として扱う。** squatting対策という目的上、検証手段の無いエントリを黙って許可する弱いフォールバックは設けていない。

fingerprintの取得・更新は`just update-android-package-fingerprints`で半自動化している。多くの主要サービスがApp Links検証用に公開しているDigital Asset Links（`assetlinks.json`）の情報を転記する仕組みで、公開していないサービスは自動取得できず手動確認に委ねる（該当エントリは確認できるまで候補が出ない状態が続く）。

**注意（Android 11+のパッケージ可視性）:** Android 11以降は、他アプリのパッケージ情報取得（署名証明書の取得を含む）がマニフェストで宣言していない限り制限される。AndroidManifest.xmlの`<queries>`で可視性を宣言する必要があり、個別パッケージ名の列挙ではなく、ランチャーアイコンを持つアプリ全般（`ACTION_MAIN`/`CATEGORY_LAUNCHER`のintent-filterを宣言するアプリ）に可視性を与える`<intent>`方式を採用している。個別列挙にすると`package_domains.json`更新のたびに同期が必要になる上、将来ユーザーが任意のインストール済みアプリをパスワードエントリに紐付けられるようにする場合（未知のパッケージが対象になる）は個別列挙では原理的に対応できないため。

これは1-3節で`QUERY_ALL_PACKAGES`を避けた判断の延長にあるが、限界も把握しておく必要がある。`ACTION_MAIN`/`CATEGORY_LAUNCHER`はほぼ全てのアプリが宣言しているため、フィンガープリンティング対策としての実効性は`QUERY_ALL_PACKAGES`とさほど変わらない、という点はAndroidコミュニティでも指摘されている既知の限界である。実際に差が出るのはGoogle Playにおける審査・開示上の扱いであり、実効的な閲覧能力の差はほぼ無い。kuraがこの方式を選ぶ理由は「情報を外部に送信しないこと」（OSSかつバックエンド・テレメトリを持たないため検証可能）にあり、「原理的に閲覧できないから安全」という主張ではない。
