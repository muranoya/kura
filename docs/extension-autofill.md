...
**マッチ対象となるURLは エントリの `typed_value.url`（ログインURL）に加え、URL型カスタムフィールド（`field_type == "url"`）の値も含まれる。** これにより、1つのログインエントリに複数のサイトURLを紐付けることができる（例: メインURLとは別に、リカバリ用URLや関連サービスのURLをカスタムフィールドに登録すれば、それらのサイトでも同じエントリが候補に出る）。マッチング処理は`vault-core/src/api/entries.rs`の`api_list_login_candidates`で一元化されている。
...
## 3-8. カスタムフィールドのセレクタベースオートフィル

username/password以外の入力（例: AWSログイン画面のAccount ID or alias）をオートフィル対象にする機能。パターンDB（3-3節）とは独立した並行の仕組みで、カスタムフィールド自体にDOM要素とのマッチ条件（タグ名・name/id/type属性）を持たせ、vaultファイルに永続化するエントリ固有の方式を採る。

詳細は [`docs/extension-custom-field-autofill.md`](extension-custom-field-autofill.md) を参照。
