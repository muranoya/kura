...
> **実装済み:** パターンDBの `match.strict_subdomain` フラグにより、サイト固有の厳密マッチングを設定できる。
...
autofillターゲットと判定されたフィールドにフォーカスすると、フィールド右端・縦中央にインラインアイコン（鍵マーク）が表示される。クリックすると候補ドロップダウンを表示する。アイコンはShadow DOM内にレンダリングされ、ページのCSSと干渉しない。

### 1-4.4 ドロップダウンの表示/非表示制御

以下の表の通り動作する。

| イベント | 動作 |
|---------|------|
| inputフィールドへのfocus（フィールドが空） | 候補があればドロップダウンを表示、インラインアイコンも表示 |
| inputフィールドへのfocus（フィールドに値あり） | ドロップダウンは表示しない、インラインアイコンのみ表示 |
| インラインアイコンをクリック | 候補ドロップダウンを表示 |
| inputフィールドからのblur | 200ms遅延後にドロップダウンとインラインアイコンを非表示（クリック猶予） |
| Escape | ドロップダウンを即座に非表示（インラインアイコンは維持） |
| ページスクロール | 位置を更新（大きく移動した場合は非表示） |

### 1-4.5 ブラウザ内蔵autofillとの共存
...
// vaultアンロック時：全タブのContent Scriptに通知（ロック状態UIの解除）
function onVaultUnlocked() {
  chrome.tabs.query({}, (tabs) => {
    tabs.forEach(tab => {
      if (tab.id) {
        chrome.tabs.sendMessage(tab.id, { type: 'AUTOFILL_VAULT_UNLOCKED' });
      }
    });
  });
}
...
|----------------|---------------|-----------|----------------|
| username | `username` | `email`, `tel` | `^(user\|email\|login\|account\|phone\|id)` |
| password | `current-password` | `password` | `^(pass\|pwd\|senha\|contrase)` |
| totp | `one-time-code` | `number`, `tel` | `^(otp\|totp\|code\|token\|verify\|mfa\|2fa)` |
...
> **実装済み:** パターンDBシステムが実装済み。パターンマッチング → ヒューリスティックのフォールバック順で動作する。開発者モード（パターンテスト）はポップアップの設定画面に実装済み。

サイト固有のフォーム検出ルールをJSONで管理するシステム。ヒューリスティック検出では対応できないサイトに対して、セレクタベースのフォーム定義を提供する。

**実装:**
- `extension/src/content/pattern-matcher.ts` — パターンマッチングエンジン（URL照合、condition評価、フィールド解決）
- `extension/src/content/pattern-detector.ts` — パターンベースのフォーム検出
- `extension/patterns/sites/` — サイト固有パターンJSON
- `extension/scripts/generate-patterns.ts` — ビルド時にパターンをマージして `patterns-data.generated.ts` を生成
...
> **注意:** `element.focus()` の呼び出しは Content Script の `focus` イベントリスナーを再トリガーする。これは `isFillingInProgress` フラグ（Section 3-2.1 参照）で防止している。
